...
- Сохранённое последнее сообщение с расписанием тихо обновляется каждые 15 минут, без отдельного уведомления Telegram.
- Отслеживаются изменения и в обычном расписании из XLSX, и в уведомлениях о заменах из DOCX.
- При новом изменении бот отправляет отдельное сообщение только подписанным пользователям соответствующей группы.
...
- Таблицы для отслеживания изменений Worker создаёт сам при первом запуске, поэтому старую D1-схему повторно выполнять не требуется.
