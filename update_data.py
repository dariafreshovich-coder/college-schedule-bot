...
def main() -> None:
    ...
    # Keep the previous timestamp when the actual schedule did not change.
    # This prevents GitHub Actions and Workers Builds from creating a new
    # deployment every 15 minutes just because generated_at changed.
    if output.exists():
        try:
        ...
        if isinstance(previous, dict):
            ...
            current_content = {key: value for key, value in result.items() if key != "generated_at"}
            if previous_content == current_content and previous.get("generated_at"):
